...
#import "HomeDisplayViewController.h"
#import "HomeFlowLayout.h"
#import "UIView+Frame.h"
#import "HomeDisplayTitleLabel.h"
#import "HomeDisplayViewHeader.h"

static NSString * const identify = @"CONTENTCELL";

@interface HomeDisplayViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UIScrollViewDelegate>
{
    UIColor *_norColor;
    UIColor *_selColor;
}
...
@implementation HomeDisplayViewController
...
- (UIScrollView *)contentScrollView
{
    if (_contentScrollView == nil) {
        ...
        // 注册cell
        [_contentScrollView registerClass:[UICollectionViewCell class] forCellWithReuseIdentifier:identify];
...
- (void)viewDidLayoutSubviews
{
    ...
    if (_isInitial == NO) {
        ...
        CGFloat titleY = _isfullScreen?STATUSBAR_H: NAVBAR_IPHONEX_H;
        
        // 是否占据全屏
        if (_isfullScreen) {
            
            // 整体contentView尺寸
            self.contentView.frame = CGRectMake(0, 0, SCREEN_W, SCREEN_H);
            self.contentView.backgroundColor = [UIColor orangeColor];
            
            // 顶部标题View尺寸
            self.titleScrollView.frame = CGRectMake(0, titleY, SCREEN_W, self.titleHeight);
...
//        self.titleScrollView.backgroundColor = [UIColor redColor];
        ...
        self.contentScrollView.frame = CGRectMake(0, contentY, SCREEN_W, contentH);
    }
}
...
- (void)setUpTitleWidth
{
    ...
    CGFloat titleMargin = (SCREEN_W - totalWidth) / (count + 1);
...
- (void)setUpAllTitle
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        // 标题label
        UILabel *label = [[HomeDisplayTitleLabel alloc] init];
//        label.backgroundColor = Randon_Color;
        label.tag = i;
    ...
    _contentScrollView.contentSize = CGSizeMake(count * SCREEN_W, 0);
    
}

#pragma mark - 标题效果渐变方法
// 设置标题颜色渐变
- (void)setUpTitleColorGradientWithOffset:(CGFloat)offsetX rightLabel:(HomeDisplayTitleLabel *)rightLabel leftLabel:(HomeDisplayTitleLabel *)leftLabel
{
    if (_isShowTitleGradient == NO) return;
    
    // 获取右边缩放
    CGFloat rightSacle = offsetX / SCREEN_W - leftLabel.tag;
...
- (void)setUpUnderLineOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel
{
    ...
    // 计算当前下划线偏移量
    CGFloat underLineTransformX = offsetDelta * centerDelta / SCREEN_W;
    
    // 宽度递增偏移量
    CGFloat underLineWidth = offsetDelta * widthDelta / SCREEN_W;
...
- (void)setUpCoverOffset:(CGFloat)offsetX rightLabel:(UILabel *)rightLabel leftLabel:(UILabel *)leftLabel
{
    ...
    // 计算当前下划线偏移量
    CGFloat coverTransformX = offsetDelta * centerDelta / SCREEN_W;
    
    // 宽度递增偏移量
    CGFloat coverWidth = offsetDelta * widthDelta / SCREEN_W;
...
{
    for (HomeDisplayTitleLabel *labelView in self.titleLabels) {
...
- (void)setUpUnderLine:(UILabel *)label
{
    ...
    // 点击时候需要动画
    [UIView animateWithDuration:0.25 animations:^{
        if (self.isUnderLineEqualTitleWidth) {
            self.underLine.width = titleBounds.size.width;
        } else {
            self.underLine.width = label.width;
        }
...
- (void)setLabelTitleCenter:(UILabel *)label
{
    ...
    // 设置标题滚动区域的偏移量
    CGFloat offsetX = label.center.x - SCREEN_W * 0.5;
    
    if (offsetX < 0) {
        offsetX = 0;
    }
    
    // 计算下最大的标题视图滚动区域 增加右侧加号区域
    CGFloat maxOffsetX = self.titleScrollView.contentSize.width - SCREEN_W + _titleMargin + 44;
...
//    NSInteger screenWInt = SCREEN_W;
//
//    NSInteger extre = offsetXInt % screenWInt;
//
//    if (extre > SCREEN_W * 0.5)
//    {
//        // 往右边移动
//        offsetX = offsetX + (SCREEN_W - extre);
//        _isAniming = YES;
//        [self.contentScrollView setContentOffset:CGPointMake(offsetX, 0) animated:YES];
//
//
//    }
//    else if (extre < SCREEN_W * 0.5 && extre > 0)
//    {
//        _isAniming = YES;
//        // 往左边移动
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    // 获取左边角标
    NSInteger leftIndex = offsetX / SCREEN_W;
    
    // 左边按钮
    HomeDisplayTitleLabel *leftLabel = self.titleLabels[leftIndex];
    
    // 右边角标
    NSInteger rightIndex = leftIndex + 1;
    
    // 右边按钮
    HomeDisplayTitleLabel *rightLabel = nil;
