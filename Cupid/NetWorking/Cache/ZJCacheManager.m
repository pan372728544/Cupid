...
+ (ZJCacheManager *)shareManager {
    static ZJCacheManager *_ZJCacheManager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _ZJCacheManager = [[ZJCacheManager alloc] init];
    });
    return _ZJCacheManager;
}
...
- (void)cacheResponseObject:(id)responseObject
    ...
    if (error == nil) {
        //缓存到内存中
        [ZJMemoryCache writeData:responseObject forKey:hash];
        ...
        if (!directoryPath) {
            directoryPath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:@"ZJNetworking"] stringByAppendingPathComponent:@"networkCache"];
            [[NSUserDefaults standardUserDefaults] setObject:directoryPath forKey:cacheDirKey];
            [[NSUserDefaults standardUserDefaults] synchronize];
        }
        [ZJDiskCache writeData:data toDir:directoryPath filename:hash];
        
        [[ZJLRUManager shareManager] addFileNode:hash];
    }
...
- (id)getCacheResponseObjectWithRequestUrl:(NSString *)requestUrl
    ...
    //先从内存中查找
    cacheData = [ZJMemoryCache readDataWithKey:hash];
    
    if (!cacheData) {
        NSString *directoryPath = [[NSUserDefaults standardUserDefaults] objectForKey:cacheDirKey];
        
        if (directoryPath) {
            cacheData = [ZJDiskCache readDataFromDir:directoryPath filename:hash];
            
            if (cacheData) [[ZJLRUManager shareManager] refreshIndexOfFileNode:hash];
        }
    }
...
- (void)storeDownloadData:(NSData *)data
    ...
    if (!directoryPath) {
        directoryPath = [[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES)[0] stringByAppendingPathComponent:@"ZJNetworking"] stringByAppendingPathComponent:@"download"];
    ...
    [ZJDiskCache writeData:data toDir:directoryPath filename:fileName];
...
- (NSURL *)getDownloadDataFromCacheWithRequestUrl:(NSString *)requestUrl {
    ...
    if (directoryPath) data = [ZJDiskCache readDataFromDir:directoryPath filename:fileName];
...
- (NSUInteger)totalCacheSize {
    ...
    return [ZJDiskCache dataSizeInDir:diretoryPath];
}
...
- (void)clearDownloadData {
    ...
    [ZJDiskCache clearDataIinDir:diretoryPath];
}
...
- (void)clearTotalCache {
    ...
    [ZJDiskCache clearDataIinDir:directoryPath];
}

- (void)clearLRUCache {
    if ([self totalCacheSize] > diskCapacity) {
        NSArray *deleteFiles = [[ZJLRUManager shareManager] removeLRUFileNodeWithCacheTime:cacheTime];
        NSString *directoryPath = [[NSUserDefaults standardUserDefaults] objectForKey:cacheDirKey];
        if (directoryPath && deleteFiles.count > 0) {
            [deleteFiles enumerateObjectsUsingBlock:^(NSString *obj, NSUInteger idx, BOOL * _Nonnull stop) {
                ...
                [ZJDiskCache deleteCache:filePath];
            }];
